...
HALT=f'{B}/slate/HALT'          # written by qa.py when the sampled failure rate crosses its threshold
...
run={}
while True:
    # THE QA GATE. A halt means the sample said this batch is casting badly; finishing the other 39
    # would just produce 39 videos with the same defect. Renders already running are left to finish.
    if os.path.exists(HALT):
        h=json.load(open(HALT))
        print(f"[{time.strftime('%H:%M:%S')}] HALTED by QA - {len(h['failed'])}/{len(h['checked'])} "
              f"sampled episodes failed ({h['rate']:.0%}, threshold {h['threshold']:.0%}). "
              f"Failing: {', '.join(h['failed'])}",flush=True)
        print('  fix the cause, then: python3 slate/qa.py --clear-halt',flush=True)
        for ep,pr in list(run.items()):
            pr.wait(); print(f'  {ep} finished before the halt took effect',flush=True)
        break
