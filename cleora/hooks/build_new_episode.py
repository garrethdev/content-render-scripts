...
def fetch_row(cid):
    r=subprocess.run(['curl','-s','--cacert',CA,f'{SB}?content_id=eq.{cid}&select=content_id,title,story_key,hook_text,script,edl,script_status',
...
for k in ('hook','story','turn','closer'):
    ...
    if not os.path.exists(raw) or os.path.getsize(raw)<1000:
        subprocess.run(['curl','-sS','--cacert',CA,'-o',raw,tts[k]],check=True)
    ...
    if not os.path.exists(iso) or os.path.getsize(iso)<1000:
        if EL:
            r=subprocess.run(['curl','-sS','--cacert',CA,'-X','POST','https://api.elevenlabs.io/v1/audio-isolation',
...
def dl(clip_key,url):
    ...
    if not os.path.exists(p) or os.path.getsize(p)<10000:
        subprocess.run(['curl','-sS','--cacert',CA,'-o',p,url],check=True)
    return p
...
def opener_set():
    ...
    try:
        r=subprocess.run(['curl','-s','--cacert',CA,url,'-H',f'apikey: {KEY}','-H',f'Authorization: Bearer {KEY}'],capture_output=True,text=True,timeout=30)
