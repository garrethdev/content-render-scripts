...
sys.path.insert(0,os.path.expanduser('~/cleora-render/OpenMontage'))
...
EP1A=os.path.expanduser('~/cleora-render/cleora-ep01/assets')
...
def validate(name,obj):
    _sp=os.path.expanduser(f'~/cleora-render/OpenMontage/schemas/artifacts/{name}.schema.json')
    if os.path.exists(_sp):
        try: jsonschema.validate(obj, json.load(open(_sp)))
        except Exception as _e: print(f'[schema warn] {name}: {_e}')
    return obj
...
FP=os.path.expanduser('~/Library/Fonts/LilitaOne-Regular.ttf')
W=int(os.environ.get('CLEORA_W','1080')); H=int(os.environ.get('CLEORA_H','1920'))
SCALE=H/1920.0
IW=(int(W*4/3)//2)*2; IH=(int(H*4/3)//2)*2
...
def music_for(ep):
    ...
    try:
        ...
        beds=[]
    if not beds: return MUSIC if os.path.exists(MUSIC) else None
...
def build(ep):
    ...
    MUSIC_LUFS=-34
    _bed=music_for(ep)
    if _bed and os.path.exists(_bed):
        print(f'{ep}: music bed {os.path.basename(_bed)}')
        ...
        print(f'{ep}: no music bed - VOICE ONLY -> -14 LUFS')
        run(['ffmpeg','-y','-loglevel','error','-i',str(A/'vo_voice.wav'),'-af','loudnorm=I=-14:LRA=11:TP=-1.5','-ar','48000',str(A/'vo_final.wav')])
    ...
    for i,(name,vst,end,zx,tx,ty,srcp) in enumerate(segs,1):
        ...
        vf=f'scale={IW}:{IH}:force_original_aspect_ratio=increase,crop={IW}:{IH}'  # 1080 output; lighter intermediate = ~2x faster derive for the parallel batch
        if sl<1.0: vf+=f',setpts=PTS/{sl}'
        vf+=f',setpts=PTS/1.1,fps=24,zoompan=z=\'{zx}\':x=\'{tx}*iw*(1-1/zoom)\':y=\'{ty}*ih*(1-1/zoom)\':d=1:s={W}x{H}:fps=24'
    ...
    _cr=CardRenderer(FP,max(12,int(62*SCALE)),max_line_px=int(800*SCALE))
    ...
    hy=int(HOOK_Y.get(ep,300)*SCALE); hy=max(10, min(hy, H-bh-10))
    ...
    edd={"version":"1.0","render_runtime":"ffmpeg","renderer_family":"documentary-montage","cuts":cuts,
      ...
      "metadata":{"compose_target":{"width":W,"height":H,"fit":"cover"},"hook_text":HOOKS[ep]}}
