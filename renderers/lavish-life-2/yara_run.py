...
OUT = os.path.expanduser("~/Claude/jessicas-aesthetic/yara-dump/out")
...
PROMPTS_FILE = sys.argv[1] if len(sys.argv) > 1 else os.path.expanduser(
    "~/Claude/jessicas-aesthetic/images/yara-dump/dump_prompts_partial24.json")
...
def gen(item):
    ...
    for attempt in range(3):
        try:
            ...
            fn = os.path.join(OUT, f"dump_{idx:02d}.jpg")
            urllib.request.urlretrieve(url, fn)
            return f"OK  dump_{idx:02d}  ({scene[:40]})"
        except Futed:
            print(f"dump_{idx:02d} attempt {attempt+1} timeout, retry", flush=True)
        except Exception as e:
            msg = str(e)[:110]
            print(f"dump_{idx:02d} attempt {attempt+1} err: {msg}", flush=True)
            if "Exhausted balance" in msg or "locked" in msg:
                return f"BALANCE  dump_{idx:02d}"
    return f"FAIL dump_{idx:02d}"
