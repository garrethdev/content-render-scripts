...
OUT = os.path.expanduser("~/Claude/jessicas-aesthetic/images/v3")
...
PROMPTS_FILE = sys.argv[1] if len(sys.argv) > 1 else os.path.expanduser(
    "~/Claude/jessicas-aesthetic/images/v3_prompts.json")
...
def gen(item):
    ...
    for attempt in range(3):
        try:
            ...
            fn = os.path.join(OUT, f"v3_{idx:02d}.jpg")
            urllib.request.urlretrieve(url, fn)
            return f"OK  v3_{idx:02d}  ({scene[:40]})"
        except Futed:
            print(f"v3_{idx:02d} attempt {attempt+1} timeout, retry", flush=True)
        except Exception as e:
            msg = str(e)[:110]
            print(f"v3_{idx:02d} attempt {attempt+1} err: {msg}", flush=True)
            if "Exhausted balance" in msg or "locked" in msg:
                return f"BALANCE  v3_{idx:02d}"
    return f"FAIL v3_{idx:02d}"
