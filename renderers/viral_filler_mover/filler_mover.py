...
def choose_hook_y_frac(face, caption_bands, reserve, default=0.06):
    """Pick the hook's top (as a frame fraction) so the block does NOT cover the
    face. Prefer the headroom ABOVE the face; else tuck it BELOW the face and
    above any existing bottom caption; else sit at the very top (least-bad)."""
    m = 0.02
    if not face or face.get("present_frac", 0) < 0.12:
        return default
    ft, fb = face["top"], face["bottom"]
    if 0.045 + reserve <= ft - m:                       # A) clear headroom above the face
        return 0.05
    cap_top = min([b[0] for b in (caption_bands or []) if b[0] > fb], default=0.97)
    floor = min(cap_top, 0.97) - m                       # B) below face, above bottom captions
    if floor - reserve >= fb + m:
        return round(floor - reserve, 4)
    return 0.035                                          # C) fall back to the very top
...
def build_caption_png(text, fmt, seed, out, style=CAPTION_STYLE,
    ...
    for fs in range(START_FS, MIN_FS - 1, -3):
        ...
        if top_y + L["total_h"] / OH <= face["top"] - m:   # fits above the face at top
            y_frac = top_y; break
    if L is None:
        L = layout(MIN_FS)
    if face_on and not (top_y + L["total_h"] / OH <= face["top"] - m):
        # even at the floor it won't clear the face from the top -> place below it
        L = layout(MIN_FS)
