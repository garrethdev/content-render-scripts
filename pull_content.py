...
Two jobs:
  ...
  2. AUDIT — check every row against the scheduler-ready checklist and report blockers,
             so you can confirm the 101 is genuinely prepped before release.
...
Usage:
  python pull_content.py --audit                       # prep report, all lanes
  python pull_content.py --audit --char2-batch B       # scope char2 to batch B
  python pull_content.py                                # download all finished media + manifest
  python pull_content.py --manifest                     # manifest.csv only
  python pull_content.py --lane char3_mito              # one lane
  python pull_content.py --ready                         # only released (scheduler_ready=true)

Runs standalone on any machine (python3 only):
  python pull_content.py --audit --key <SUPABASE_SECRET_KEY>
Key also read from env CAROUSEL_SUPABASE_SECRET_KEY.
"""
